...
DATE_FORMATS = {#1/9/2026 TODO - Update for EDIFACT
    '0020': '%y%m%d',
    '0030': '%y%m%d',
    '0040': '%Y%m%d',
    '0050': '%Y%m%d',
...
START_SEGMENTS = {#1/9/2026 TODO - Update for EDIFACT
                 # Record no # Doc Date
    '861':'BRA', # 1         #   2
    '810':'BIG', # 1         #   1
    '824':'BGN', # 2         #   3
...
class EdiEdifact(EdiBase):
    ...
    def handle_release(self, segment, state):
        if state["part_record"]:
            qty, fcst, timing, reldate = self.universal_element_extract(segment,[1,2,3,4], date=True)
            state["rel_class"] = EdifactReleaseDetails(reldate,qty,fcst,timing)
...
class EdifactReleaseDetails(EdiReleaseDetails):
    def __init__(self, date: str, quantity: str, rel_type: str, rel_timing: str, **kwargs):
        super().__init__('EDIFACT', date, quantity)
    ...
    def __repr__(self):
        return f"EdifactReleaseDetail(date={self.date}, quantity={self.quantity}, rel_type={self.rel_type}, rel_timing={self.rel_timing})"